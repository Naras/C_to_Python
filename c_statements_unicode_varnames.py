import re
stmt_comment = "if(ವಿಭಕ್ತಿ->stype =='1'){  /* blah \n bleh */;\nyes=findverb;}\n/* foo  */\n"
# stmt_comment = "if(ವಿಭಕ್ತಿ->stype =='1'){ /* blah */yes=findverb;}/*  */"
# stmt_comment = "if(ವಿಭಕ್ತಿ->stype =='1') yes=findverb;"
...
stmt_func_decl_complex1 = "int choice(char type,unsigned char *word,unsigned char voice[],int pos,VIBAK *ವಿಭಕ್ತಿ,FILE *afp,long fl,unsigned char *VerbMean)"
...
stmt_func_def_complex1 = "int choice(char type,unsigned char *word,unsigned char voice[],int pos,VIBAK *ವಿಭಕ್ತಿ,FILE *afp,long fl,unsigned char *VerbMean){" \
                         "int yes=0,success=1;" \
                         "if((ವಿಭಕ್ತಿ->stype =='1' && strcmp(ವಿಭಕ್ತಿ->specf,'dative')==0 ) || ವಿಭಕ್ತಿ->stype =='5' || ವಿಭಕ್ತಿ->stype=='2'|| ವಿಭಕ್ತಿ->stype=='4')" \
                         "		{/* Check for case where there is only a single meaning for चतुर्ती विभक्ति */" \
                         "yes=findverb(voice,ವಿಭಕ್ತಿ->sword,ವಿಭಕ್ತಿ,afp,fl,VerbMean);}return success;}"
stmt_func_def_complex2 = "int choice(char type,unsigned char *word,unsigned char voice[],int pos,VIBAK *ವಿಭಕ್ತಿ,FILE *afp,long fl,unsigned char *VerbMean){while(1){" \
                         "if((ವಿಭಕ್ತಿ->stype =='1' && strcmp(ವಿಭಕ್ತಿ->specf,'dative')==0 ) || ವಿಭಕ್ತಿ->stype =='5' || ವಿಭಕ್ತಿ->stype=='2'|| ವಿಭಕ್ತಿ->stype=='4'){" \
                         "/* Check for case where there is only a single meaning for चतुर्ती विभक्ति */" \
                         "yes=findverb(voice,ವಿಭಕ್ತಿ->sword,ವಿಭಕ್ತಿ,afp,fl,VerbMean);if(ವಿಭಕ್ತಿ->stype=='2' && ವಿಭಕ್ತಿ->matnoun !=1 )" \
                         "{switch(ವಿಭಕ್ತಿ->spos){" \
                         "case 0:if(ವಿಭಕ್ತಿ->semlinga==0)strcat(ವಿಭಕ್ತಿ->arthaword,'सा ');if(ವಿಭಕ್ತಿ->semlinga==1)strcat(ವಿಭಕ್ತಿ->arthaword,'सः ');if(ವಿಭಕ್ತಿ->semlinga==2)strcat(ವಿಭಕ್ತಿ->arthaword,'त्त ');break;" \
                         "case 1:strcat(ವಿಭಕ್ತಿ->arthaword,'तन्निष्ठकर्मतानिरूपक ');break;" \
                         "case 2:strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठकरणतानिरूपक ');break;" \
                         "case 3:strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठसम्प्रदानतानिरूपक ');break;" \
                         "case 4:strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठापादानतानिरूपक ');break;" \
                         "case 5:strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठाधिकरणतानिरूपक ');break;}}" \
                         "if(ವಿಭಕ್ತಿ->stype == '2' || ವಿಭಕ್ತಿ->stype =='4' || ವಿಭಕ್ತಿ->stype=='5')success= 0;}if(ವಿಭಕ್ತಿ->stype =='1' && (strcmpi(ವಿಭಕ್ತಿ->specf,'object')==0)){" \
                         "       /* Check for case where there is only a single meaning for द्वितीया विभक्ति */" \
                         "yes=findverb(voice,ವಿಭಕ್ತಿ->sword,ವಿಭಕ್ತಿ,afp,fl,VerbMean);}" \
                         "/* If not in above case following steps lead to menu display for   selection based on type of vibhakti */ " \
                         "if(ವಿಭಕ್ತಿ->stype =='1') { switch(ವಿಭಕ್ತಿ->spos) {" \
                         "case 0:if(strcmpi(voice,'kartari') ==0)strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->sword);if(strcmpi(voice,'karmani') ==0){strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword);strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठकर्तृतानिरूपक ');}break;" \
                         "case 1:if(strcmpi(voice,'kartari') ==0){strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword);strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठकर्मतानिरूपक ');}if(strcmpi(voice,'karmani') ==0){strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->sword);}break; " \
                         "case 2:strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword);strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठकरणतानिरूपक ');break; " \
                         "case 3:strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword);strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठसम्प्रदानतानिरूपक ');break; " \
                         "case 4:strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword);strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठापादानतानिरूपक ');break; " \
                         "case 6:strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword);strcat(ವಿಭಕ್ತಿ->arthaword,'सम्बन्धि ');break; " \
                         "case 5:strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword);strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठाधिकरणतानिरूपक ');break;} } " \
                         "if (ವಿಭಕ್ತಿ->next != NULL)ವಿಭಕ್ತಿ=ವಿಭಕ್ತಿ->next; else break;}return success;}"
...
stmt_if_assign4 = 'if(strcmp(word,list1[i])==0) {if(linga==0) strcpy(message,"समवेता");  ' \
                  'if(linga==1) strcpy(message,"समवेतः");if(linga==2) strcpy(message,"समवेतं");' \
                 'strcpy(vword,ವಿಭಕ್ತಿ->bword);  strcat(vword,message);  strcpy(ವಿಭಕ್ತಿ->arthaword,vword);return 1; } '
stmt_strcmp_cpy_cat = 'if(strcmpi(voice,"karmani") ==0) \
      					{ \
      						strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword); \
      						strcat(ವಿಭಕ್ತಿ->arthaword,"निष्ठकर्तृतानिरूपक ");if(strcmpi(voice,"karmani") ==0) strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword);}'
...
stmt_switch_case2 = 'switch(ವಿಭಕ್ತಿ->spos) {case 0:if(strcmpi(voice,"kartari") ==0) strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->sword);' \
                    'if(strcmpi(voice,"karmani") ==0) strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->sword);' \
                    'case "1": if(strcmpi(voice,"karmani") ==0)strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->sword); break; case 3: j = "ikh"}'
stmt_switch_case22 = 'switch(ವಿಭಕ್ತಿ->spos) {case 0:i = 1; break; case "1": choice = 3; break; case 3: j = "ikh"}'
stmt_switch_case3 = 'switch(ವಿಭಕ್ತಿ->spos) {' \
                    'case 0:if(strcmpi(voice,"kartari") ==0)strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->sword);' \
                    'if(strcmpi(voice,"karmani") ==0){strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword);strcat(ವಿಭಕ್ತಿ->arthaword,"निष्ठकर्तृतानिरूपक ");}break;' \
                    'case 1:if(strcmpi(voice,"kartari") ==0){strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword);strcat(ವಿಭಕ್ತಿ->arthaword,"निष्ठकर्मतानिरूपक ");}' \
                    'if(strcmpi(voice,"karmani") ==0){strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->sword);}break;' \
                    ' case 2:strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword);strcat(ವಿಭಕ್ತಿ->arthaword,"निष्ठकरणतानिरूपक ");break;' \
                    ' case 3:strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword);strcat(ವಿಭಕ್ತಿ->arthaword,"निष्ठसम्प्रदानतानिरूपक ");break;' \
                    ' case 4:strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword);strcat(ವಿಭಕ್ತಿ->arthaword,"निष्ठापादानतानिरूपक ");break; ' \
                    'case 6:strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword);strcat(ವಿಭಕ್ತಿ->arthaword,"सम्बन्धि ");break; ' \
...
stmt_if_while_complex1 = "if((ವಿಭಕ್ತಿ->stype =='1' && strcmp(ವಿಭಕ್ತಿ->specf,'dative')==0 ) || ವಿಭಕ್ತಿ->stype =='5' || ವಿಭಕ್ತಿ->stype=='2'|| ವಿಭಕ್ತಿ->stype=='4')" \
                      "		{/* Check for case where there is only a single meaning for चतुर्ती विभक्ति */" \
                      "yes=findverb(voice,ವಿಭಕ್ತಿ->sword,ವಿಭಕ್ತಿ,afp,fl,VerbMean);}"
stmt_while_complex2 = "while(1)  { " \
                      ...
                      "{ /* Check for case where there is only a single meaning for चतुर्ती विभक्ति */  " \
                      "yes=findverb(voice,ವಿಭಕ್ತಿ->sword,ವಿಭಕ್ತಿ,afp,fl,VerbMean);   " \
                      "if(ವಿಭಕ್ತಿ->stype=='2' && ವಿಭಕ್ತಿ->matnoun !=1 )  {" \
                      "   switch(ವಿಭಕ್ತಿ->spos)   {" \
                      "  case 0:   if(ವಿಭಕ್ತಿ->semlinga==0)    strcat(ವಿಭಕ್ತಿ->arthaword,'सा ');" \
                      "   if(ವಿಭಕ್ತಿ->semlinga==1)    strcat(ವಿಭಕ್ತಿ->arthaword,'सः ');" \
                      "   if(ವಿಭಕ್ತಿ->semlinga==2)    strcat(ವಿಭಕ್ತಿ->arthaword,'त्त ');   break;  " \
                      "case 1:   strcat(ವಿಭಕ್ತಿ->arthaword,'तन्निष्ठकर्मतानिरूपक ');   break;  " \
                      "case 2:   strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठकरणतानिरूपक ');   break;  " \
                      "case 3:   strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठसम्प्रदानतानिरूपक ');   break;  " \
                      "case 4:   strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठापादानतानिरूपक ');   break;  " \
                      "case 5:   strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठाधिकरणतानिरूपक ');   break;   }" \
                      "  }  if(ವಿಭಕ್ತಿ->stype == '2' || ವಿಭಕ್ತಿ->stype =='4' || ವಿಭಕ್ತಿ->stype=='5')   success= 0;  } " \
                      "if(ವಿಭಕ್ತಿ->stype =='1' && (strcmpi(ವಿಭಕ್ತಿ->specf,'object')==0)) {" \
                      "    /* Check for case where there is only a single meaning for द्वितीया विभक्ति */  " \
                      "yes=findverb(voice,ವಿಭಕ್ತಿ->sword,ವಿಭಕ್ತಿ,afp,fl,VerbMean); }}"
stmt_while_complex3 = "while(1){" \
                      ...
                      "case 6:strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword);strcat(ವಿಭಕ್ತಿ->arthaword,'सम्बन्धि ');break; " \
                      "case 5:strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword);strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठाधिकरणतानिरूपक ');break;} } " \
                      "if (ವಿಭಕ್ತಿ->next != NULL)ವಿಭಕ್ತಿ=ವಿಭಕ್ತಿ->next; else break;}"
stmt_include = "#include <stdio.h>\n"
stmt_include2 = '#include "sengen1.h"\n #include "data.h"\n'
stmt_define = "#define KARTHARI    0 #define KARMANI     1 #define FULLSTOP  '।' #define eof       255 "
...
stmt_var_decl_array = "unsigned char list[]={'पूर्वाण्ह', 'अपराण्ह', 'दिन', 'रात्रि', 'अहोरात्र', 'सप्ताह', 'पक्ष', 'मास', 'संवत्सर'};"
stmt_func_def_vibmenu_full = "int choice(char type,unsigned char *word,unsigned char voice[],int pos,VIBAK *ವಿಭಕ್ತಿ,FILE *afp,long fl,unsigned char *VerbMean) { int yes=0,success=1;  while(1) { if((ವಿಭಕ್ತಿ->stype =='1' && strcmp(ವಿಭಕ್ತಿ->specf,'dative')==0 ) || ವಿಭಕ್ತಿ->stype =='5' || ವಿಭಕ್ತಿ->stype=='2'|| ವಿಭಕ್ತಿ->stype=='4') { /* Check for case where there is only a single meaning for चतुर्ती विभक्ति */ yes=findverb(voice,ವಿಭಕ್ತಿ->sword,ವಿಭಕ್ತಿ,afp,fl,VerbMean);  if(ವಿಭಕ್ತಿ->stype=='2' && ವಿಭಕ್ತಿ->matnoun !=1 ) { switch(ವಿಭಕ್ತಿ->spos) { case 0: if(ವಿಭಕ್ತಿ->semlinga==0) strcat(ವಿಭಕ್ತಿ->arthaword,'सा '); if(ವಿಭಕ್ತಿ->semlinga==1) strcat(ವಿಭಕ್ತಿ->arthaword,'सः '); if(ವಿಭಕ್ತಿ->semlinga==2) strcat(ವಿಭಕ್ತಿ->arthaword,'त्त '); break; case 1: strcat(ವಿಭಕ್ತಿ->arthaword,'तन्निष्ठकर्मतानिरूपक '); break; case 2: strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठकरणतानिरूपक '); break; case 3: strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठसम्प्रदानतानिरूपक '); break; case 4: strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठापादानतानिरूपक '); break; case 5: strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठाधिकरणतानिरूपक '); break; } } if(ವಿಭಕ್ತಿ->stype == '2' || ವಿಭಕ್ತಿ->stype =='4' || ವಿಭಕ್ತಿ->stype=='5') success= 0;  } if(ವಿಭಕ್ತಿ->stype =='1' && (strcmpi(ವಿಭಕ್ತಿ->specf,'object')==0)) {        /* Check for case where there is only a single meaning for द्वितीया विभक्ति */ yes=findverb(voice,ವಿಭಕ್ತಿ->sword,ವಿಭಕ್ತಿ,afp,fl,VerbMean); }   /* If not in above case following steps lead to menu display for    selection based on type of vibhakti */  if(ವಿಭಕ್ತಿ->stype =='1')  {  switch(ವಿಭಕ್ತಿ->spos)  { case 0: if(strcmpi(voice,'kartari') ==0) strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->sword); if(strcmpi(voice,'karmani') ==0) { strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword); strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठकर्तृतानिरूपक '); } break;  case 1: if(strcmpi(voice,'kartari') ==0) { strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword); strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठकर्मतानिरूपक '); } if(strcmpi(voice,'karmani') ==0) { strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->sword); } break;  case 2: strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword); strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठकरणतानिरूपक '); break;  case 3: strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword); strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठसम्प्रदानतानिरूपक '); break;  case 4: strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword); strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठापादानतानिरूपक '); break;  case 6: strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword); strcat(ವಿಭಕ್ತಿ->arthaword,'सम्बन्धि '); break;  case 5: strcpy(ವಿಭಕ್ತಿ->arthaword,ವಿಭಕ್ತಿ->bword); strcat(ವಿಭಕ್ತಿ->arthaword,'निष्ठाधिकरणतानिरूपक '); break; }  }  if (ವಿಭಕ್ತಿ->next != NULL) ವಿಭಕ್ತಿ=ವಿಭಕ್ತಿ->next;  else  break; } return success; }"
stmt_for = 'for(i=0;i<vno;i-=3) free(words[i]);'
stmt_increment_decrement = 'i++;j--;k+=2;h-=3;'
stmt_func_def_complex3 = 'int findverb(unsigned char voice[],unsigned char *Word,VIBAK *ವಿಭಕ್ತಿ,FILE *afp,long fl,unsigned char *VerbMean)' \
                 '{int found=0,i,j,pos;int vnum=0,vno=0,linga=4;unsigned char line1[300],*words[17],message[60],word[20],vword[80],type,' \
                 'lword[30],ltype="Z",temp[4];unsigned char *list[]={"पूर्वाण्ह", "अपराण्ह", "दिन", "रात्रि", "अहोरात्र", "सप्ताह", "पक्ष", "मास", "संवत्सर"};' \
                 'unsigned char *list1[]={"ज्ञान","सुख","दुःख","इच्छा","द्वेष","प्रयत्न","धर्म","अधर्म","संस्कार","भावना","विवेक","बोध","बुद्धि","धी","प्रज्ञा",};' \
                 'fseek(afp,fl,0);}if(ವಿಭಕ್ತಿ->stype =="2") type="0";' \
                 ' while(found==0  ||(!feof(afp))){fgets(line1,299,afp);if(line1[0]==" - "){found=1;break;}vno=0;vno=split(line1,words);' \
                 'if(strcmp(words[0],"वाक्यम्")==0) {for(i=0;i<vno;i++){free(words[i]);continue;}}' \
                 'vno=0; vno=split(line1,words); ' \
                 'if(strcmp(words[0],"वाक्यम्")==0) /* Check for input sentence */ ' \
                 ...
                 '  if(linga==0) strcpy(message,"समवेता");  ' \
                 'if(linga==1) strcpy(message,"समवेतः");' \
                 'if(linga==2) strcpy(message,"समवेतं");' \
                 '  strcpy(vword,ವಿಭಕ್ತಿ->bword);  strcat(vword,message);  strcpy(ವಿಭಕ್ತಿ->arthaword,vword);' \
                 '  return 1; }  } } ' \
                 'if(type=="5") {  found=dispmea(voice,vnum,list,ವಿಭಕ್ತಿ,ltype,lword);  for(i=0;i<vno;i++) free(words[i]);  return found; } ' \
                 'if(type=="2" && ವಿಭಕ್ತಿ->stype=="2" && strcmp(Word,words[1])==0) {  found=dispmea(voice,vnum,list,ವಿಭಕ್ತಿ,ltype,lword);  ' \
                 'for(i=0;i<vno;i++) free(words[i]);  return found; } else if(type=="4" && ವಿಭಕ್ತಿ->stype=="4" && strcmp(Word,words[1])==0) ' \
                 '{  found=dispmea(voice,vnum,list,ವಿಭಕ್ತಿ,ltype,lword);  for(i=0;i<vno;i++) free(words[i]);  return found; } ' \
                 'for(i=0;i<vno;i++)  free(words[i]);};'
stmt_assignment_and_condition = 'a = b; message = "समवेतः"; if(c=fgetc(cfp)!=eof) x = y'
...
samples = [stmt_comment, stmt_var_decl_initialized, stmt_var_decl_array, stmt_assignment, stmt_func_decl_simple, stmt_func_decl_complex, stmt_func_decl_complex1, stmt_func_decl_complex2, stmt_func_def_complex1, stmt_func_def_complex2, stmt_assignment_func, stmt_if_assign, stmt_if_assign2, stmt_if_assign3, stmt_if_assign4, stmt_strcmp_cpy_cat, stmt_switch_case, stmt_switch_case1, stmt_switch_case2, stmt_switch_case22, stmt_switch_case3, stmt_while, stmt_if_while_complex1, stmt_while_complex2, stmt_while_complex3, stmt_define, stmt_include, stmt_include2, stmt_typedef_many, stmt_func_def_vibmenu_full, stmt_increment_decrement, stmt_for, stmt_func_def_complex3, stmt_assignment_and_condition, stmt_multilevel_indices_pointers, stmt_includes_defines_others, stmt_multilevel_pointers_indices_and_assigned_conditions]

IAST_Dict = {'अ': 'a', 'आ': 'ā', 'इ': 'i', 'ई': 'ī', 'उ': 'u', 'ऊ': 'ū', 'ए': 'e', 'ऐ': 'ai', 'ओ': 'o', 'औ': 'au', 'ऋ': 'ṛ', 'ॠ': 'ṝ', 'ऌ': 'ḷ', 'ॡ': 'ḹ', 'अं': 'ṃ', 'अः': 'ḥ', 'अँ': '', 'ऽ': "'", 'क': 'ka', 'ख': 'kha', 'ग': 'ga', 'घ': 'gha', 'ङ': 'ṅa', 'च': 'ca', 'छ': 'cha', 'ज': 'ja', 'झ': 'jha', 'ञ': 'ña', 'ट': 'ṭa', 'ठ': 'ṭha', 'ड': 'ḍa', 'ढ': 'ḍha', 'ण': 'ṇa', 'त': 'ta', 'थ': 'tha', 'द': 'da', 'ध': 'dha', 'न': 'na', 'प': 'pa', 'फ': 'pha', 'ब': 'ba', 'भ': 'bha', 'म': 'ma', 'य': 'ya', 'र': 'ra', 'ल': 'la', 'व': 'va', 'श': 'śa', 'ष': 'ṣa', 'स': 'sa', 'ह': 'ha'}
ITRANS_Dict = {'अ': 'a', 'आ': 'A', 'इ': 'i', 'ई': 'I', 'उ': 'u', 'ऊ': 'U', 'ए': 'e', 'ऐ': 'ai', 'ओ': 'o', 'औ': 'au', 'ऋ': 'R', 'ॠ': 'RR', 'ऌ': 'lR', 'ॡ': 'lRR', 'अं': 'M', 'अः': 'H', 'अँ': '', 'ऽ': "'", 'क': 'ka', 'ख': 'kha', 'ग': 'ga', 'घ': 'gha', 'ङ': 'Ga', 'च': 'ch', 'छ': 'chha', 'ज': 'ja', 'झ': 'jha', 'ञ': 'Ja', 'ट': 'Ta', 'ठ': 'Tha', 'ड': 'Da', 'ढ': 'Dha', 'ण': 'Na', 'त': 'ta', 'थ': 'tha', 'द': 'da', 'ध': 'dha', 'न': 'na', 'प': 'pa', 'फ': 'pha', 'ब': 'ba', 'भ': 'bha', 'म': 'ma', 'य': 'ya', 'र': 'ra', 'ल': 'la', 'व': 'va', 'श': 'za', 'ष': 'Sa', 'स': 'sa', 'ह': 'ha'}
# print(set(IAST_Dict.items()) ^ set(ITRANS_Dict.items()))
...
pattern_end_with_newline, pattern_star_slash, pattern_star_slash_semicolon = \
